...
// Modify p5 to ignore out-of-bounds positions before setting hIsDown
p5.prototype._onhstart = function(e) {
  if (!this._curElement) {
    return;
  }
  var validh;
  for (var i = 0; i < e.hes.length; i++) {
    validh = gethInfo(this._curElement.elt, e, i);
    if (validh) {
      break;
    }
  }
  if (!validh) {
    // No in-bounds (valid) hes, return and ignore:
    return;
  }
  var context = this._isGlobal ? window : this;
  var executeDefault;
  this._updateNexthCoords(e);
  this._updateNextMouseCoords(e);
  this._setProperty('hIsDown', true);
  if (typeof context.hStarted === 'function') {
    executeDefault = context.hStarted(e);
...
// positions before reporting h coordinates
//
// NOTE: _updateNexthCoords() is nearly identical, but calls a modified
// gethInfo() function below that scales the h postion with the play
// space and can return undefined
p5.prototype._updateNexthCoords = function(e) {
  var x = this.touchX;
  var y = this.touchY;
